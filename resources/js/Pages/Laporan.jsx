            ...
            <div className="max-w-7xl mx-auto">
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">Analisis pemasukan dan pengeluaran Anda.</p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col gap-8">
                        ...
                        <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-slate-100">
                            <h2 className="text-lg font-bold text-slate-800 mb-6">Tren Pemasukan & Pengeluaran</h2>
                            <div className="w-full h-[350px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={tren6Bulan} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                                        <XAxis dataKey="bulan" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} dy={10} />
                                        <YAxis tickFormatter={formatRupiahShort} axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} dx={-10} />
                                        ...
                                        <Bar dataKey="pengeluaran" name="Pengeluaran" fill="#ef4444" radius={[4, 4, 0, 0]} barSize={30} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-slate-100 flex flex-col">
                                <h2 className="text-lg font-bold text-slate-800 mb-6">Pengeluaran per Kategori Bulan Ini</h2>
                                        ...
                                        <ResponsiveContainer width="100%" height={280}>
                                            <PieChart>
                                                <Pie
                                                    ...
                                                    stroke="none"
                                                >
                                                    {sortedKategori.map((entry, index) => (
                                                        <Cell key={`cell-${index}`} fill={entry.warna || '#64748b'} />
                                                    ))}
                                                </Pie>
                            ...
                            <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-slate-100 flex flex-col">
                                <h2 className="text-lg font-bold text-slate-800 mb-6">Ringkasan Pengeluaran</h2>
                                {sortedKategori.length > 0 ? (
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="border-b border-slate-100">
                                                    <th className="pb-3 text-xs uppercase tracking-wider font-bold text-slate-500">Kategori</th>
                                                    <th className="pb-3 text-xs uppercase tracking-wider font-bold text-slate-500 text-right">Total Pengeluaran</th>
                                                    <th className="pb-3 text-xs uppercase tracking-wider font-bold text-slate-500 text-right">Persentase</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-50">
                                                {sortedKategori.map((cat, idx) => {
                                                    const pct = totalPengeluaranKategori > 0 ? (cat.total / totalPengeluaranKategori) * 100 : 0;
                                                    return (
                                                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                                            <td className="py-4">
                                                                <div className="flex items-center gap-3">
                                                                    <div className="w-3 h-3 rounded-full" style={{backgroundColor: cat.warna}}></div>
                                                                    <span className="font-semibold text-slate-700 text-sm">{cat.kategori}</span>
                                                                </div>
                                                            </td>
                                                            <td className="py-4 text-right font-medium text-slate-700 text-sm">
                                                                {formatRupiahFull(cat.total)}
                                                            </td>
                                            ...
                                            <tfoot>
                                                <tr className="border-t-2 border-slate-100">
                                                    <td className="py-4 font-bold text-slate-900 text-sm">Total Keseluruhan</td>
                                                    <td className="py-4 text-right font-extrabold text-rose-600 text-sm">
                                                        {formatRupiahFull(totalPengeluaranKategori)}
                                                    </td>
